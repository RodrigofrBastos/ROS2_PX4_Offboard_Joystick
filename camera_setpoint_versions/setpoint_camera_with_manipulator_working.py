...
class SetpointCamera(Node):
    ...
    def init_parameters(self):
        ...
        self.declare_parameter('success_z_vel', -0.7) # Vel Z (NED) após 10s em SUCCESS
    ...
    def init_ros_interfaces(self):
        ...
        self.attitude_sub = self.create_subscription(
            ...
            qos_px4)
        self.error_xy = self.create_publisher(Float64, "/pid_error_xy", 10)
        self.error_z = self.create_publisher(Float64, "/pid_error_z", 10)
        self.setpoint_z_ned_pub = self.create_publisher(Float64, "/manipulator_setpoint_z", 10)
        self.allow_move_pub = self.create_publisher(Bool, "/allow_move", 10)
        self.is_stable = self.create_publisher(Bool, "/is_stable", 10)

        self.pitch_angle = self.create_publisher(Float64, "/drone_pitch_angle", 10)
        self.roll_angle = self.create_publisher(Float64, "/drone_roll_angle", 10)
        
        self.elapsed_time_xy_pub = self.create_publisher(Float64, "/elapsed_time_xy", 10)
        self.elapsed_time_z_pub = self.create_publisher(Float64, "/elapsed_time_z", 10)

        self.timer = self.create_timer(self.timer_period, self.timer_callback)

        # ========== NOVOS PUBLISHERS PARA ANÁLISE DE LYAPUNOV ==========
        # Erros individuais de posição
        self.lyap_error_x_pub = self.create_publisher(Float64, "/lyapunov/error_x", 10)
        self.lyap_error_y_pub = self.create_publisher(Float64, "/lyapunov/error_y", 10)
        self.lyap_error_z_pub = self.create_publisher(Float64, "/lyapunov/error_z", 10)
        
        # Normas dos erros
        self.lyap_error_xy_norm_pub = self.create_publisher(Float64, "/lyapunov/error_xy_norm", 10)
        self.lyap_error_total_norm_pub = self.create_publisher(Float64, "/lyapunov/error_total_norm", 10)
        
        # Derivadas dos erros
        self.lyap_derivative_error_x_pub = self.create_publisher(Float64, "/lyapunov/derivative_error_x", 10)
        self.lyap_derivative_error_y_pub = self.create_publisher(Float64, "/lyapunov/derivative_error_y", 10)
        self.lyap_derivative_error_z_pub = self.create_publisher(Float64, "/lyapunov/derivative_error_z", 10)
        
        # Erros integrativos
        self.lyap_integral_error_x_pub = self.create_publisher(Float64, "/lyapunov/integral_error_x", 10)
        self.lyap_integral_error_y_pub = self.create_publisher(Float64, "/lyapunov/integral_error_y", 10)
        self.lyap_integral_error_z_pub = self.create_publisher(Float64, "/lyapunov/integral_error_z", 10)
        
        # Posições atuais
        self.lyap_current_pos_x_pub = self.create_publisher(Float64, "/lyapunov/current_pos_x", 10)
        self.lyap_current_pos_y_pub = self.create_publisher(Float64, "/lyapunov/current_pos_y", 10)
        self.lyap_current_pos_z_pub = self.create_publisher(Float64, "/lyapunov/current_pos_z", 10)
        
        # Setpoints desejados
        self.lyap_setpoint_x_pub = self.create_publisher(Float64, "/lyapunov/setpoint_x", 10)
        self.lyap_setpoint_y_pub = self.create_publisher(Float64, "/lyapunov/setpoint_y", 10)
        self.lyap_setpoint_z_pub = self.create_publisher(Float64, "/lyapunov/setpoint_z", 10)
        
        # Velocidades comandadas
        self.lyap_cmd_vel_x_pub = self.create_publisher(Float64, "/lyapunov/cmd_vel_x", 10)
        self.lyap_cmd_vel_y_pub = self.create_publisher(Float64, "/lyapunov/cmd_vel_y", 10)
        self.lyap_cmd_vel_z_pub = self.create_publisher(Float64, "/lyapunov/cmd_vel_z", 10)
        self.lyap_cmd_vel_norm_pub = self.create_publisher(Float64, "/lyapunov/cmd_vel_norm", 10)
        
        # Tempo
        self.lyap_dt_pub = self.create_publisher(Float64, "/lyapunov/dt", 10)
        self.lyap_timestamp_pub = self.create_publisher(Float64, "/lyapunov/timestamp", 10)
    ...
    def init_variables(self):
        ...
        self.allow_move = False  # Publica se o manipulador pode se mover ou não
        self.moving_entry_time = None  # Armazena quando entramos em MOVING
    ...
    def run_state_moving(self):
        ...
        if self.control_phase == 'XY':
            # Armazena o tempo de entrada na primeira vez que esta função é chamada
            if self.moving_entry_time is None:
                self.moving_entry_time = self.get_clock().now()

            # Calcula o tempo decorrido
            xy_elapsed_time = self.get_clock().now() - self.moving_entry_time
            xy_elapsed_seconds = xy_elapsed_time.nanoseconds / 1e9

            current_error_xy = Float64()
            current_error_xy.data = current_error_distance_xy
            self.error_xy.publish(current_error_xy)
            ...
            if current_error_distance_xy < self.threshold_distance:
                ...
                if self.success_counter >= 10:
                    ...
                    # Muda a fase
                    self.get_logger().info(f"Tempo em MOVING XY: {xy_elapsed_seconds:.2f}s")
                    
                    elapsed_time_xy_pub_msg = Float64()
                    elapsed_time_xy_pub_msg.data = xy_elapsed_seconds
                    self.elapsed_time_xy_pub.publish(elapsed_time_xy_pub_msg)
                    self.moving_entry_time = None # Reseta tempo de entrada em MOVING
        ...
        # ==========================================================
            ...
            # Calcula o tempo decorrido
            z_elapsed_time = self.get_clock().now() - self.moving_entry_time
            z_elapsed_seconds = z_elapsed_time.nanoseconds / 1e9

            current_error_z = Float64()
            current_error_z.data = current_error_distance_z
            self.error_z.publish(current_error_z)
            ...
            if abs(current_error_distance_z) <= abs(self.threshold_distance_z):
                ...
                self.success_counter += 1
                self.get_logger().info(f"Tempo em MOVING Z: {z_elapsed_seconds:.2f}s")

                elapsed_time_z_pub_msg = Float64()
                elapsed_time_z_pub_msg.data = z_elapsed_seconds
                self.elapsed_time_z_pub.publish(elapsed_time_z_pub_msg)
    ...
    def run_state_success(self):
        ...
        self.target_coor_z
        manipulator_setpoint = Float64()
        manipulator_setpoint.data = self.target_coor_z
        self.setpoint_z_ned_pub.publish(manipulator_setpoint)
        ...
        if elapsed_seconds < 10.0:
            ...
            allow_move_msg = Bool()
            allow_move_msg.data = self.allow_move
            self.allow_move_pub.publish(allow_move_msg)
        ...
        elif elapsed_seconds > 10 and elapsed_seconds < 20:
            ...
            current_pitch_deg = Float64()
            current_pitch_deg.data = np.rad2deg(current_pitch_rad)
            self.pitch_angle.publish(current_pitch_deg)

            current_roll_deg = Float64()
            current_roll_deg.data = np.rad2deg(current_roll_rad)
            self.roll_angle.publish(current_roll_deg)
